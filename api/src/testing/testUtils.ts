...
export class TestUtils {
  ...
  async createMarket(
    statement: string,
    duration: number,
    creator: string = this.randomWallet()
  ): Promise<TestMarket> {
    const response = await this.client.post<ApiResponse<TestMarket>>(
      '/markets',
      {
        statement,
        duration,
  ...
  async getMarkets(options: {
    ...
    sortBy?: string;
    sortOrder?: string;
  } = {}): Promise<{ markets: TestMarket[]; pagination: any }> {
   ...
   */
  async getMarket(marketId: string): Promise<TestMarket & { opinions: TestOpinion[] }> {
    const response = await this.client.get<
      ApiResponse<TestMarket & { opinions: TestOpinion[] }>
    >(`/markets/${marketId}`);
   ...
   */
  async getUserPortfolio(wallet: string): Promise<TestPortfolio> {
    const response = await this.client.get<ApiResponse<TestPortfolio>>(
      `/user/${wallet}`
    );
  ...
  async getUserPositions(
    ...
    limit: number = 50,
    offset: number = 0
  ): Promise<{ positions: TestPosition[]; pagination: any }> {
    const response = await this.client.get<ApiResponse<TestPosition[]>>(
      `/user/${wallet}/positions?limit=${limit}&offset=${offset}`
    );
   ...
   */
  async getSentimentHistory(limit: number = 50, offset: number = 0): Promise<any> {
    const response = await this.client.get(
      `/sentiment/history?limit=${limit}&offset=${offset}`
    );
   ...
   */
  async searchTopic(query: string, limit: number = 20): Promise<any> {
    const response = await this.client.get(
      `/sentiment/topic?q=${encodeURIComponent(query)}&limit=${limit}`
    );
   ...
   */
  validateStatement(statement: string): { valid: boolean; error?: string } {
    if (!statement) return { valid: false, error: 'Statement is required' };
   ...
   */
  validateStakeAmount(amount: number): { valid: boolean; error?: string } {
   ...
   */
  validateWallet(wallet: string): { valid: boolean; error?: string } {
   ...
   */
  randomWallet(): string {
    return `wallet_${uuidv4().slice(0, 36)}`;
  }
   ...
   */
  async testFullLifecycle(): Promise<{
    market: TestMarket;
    ...
    const opinions: TestOpinion[] = [];
    const wallets = [this.randomWallet(), this.randomWallet(), this.randomWallet()];
    for (const wallet of wallets) {
      const opinion = await this.stakeOpinion(
        market.id,
        5_000_000,
        `Opinion from ${wallet.slice(0, 10)}...`,
        wallet
      );
 ...
 */
export function createTestUtils(baseUrl?: string): TestUtils {
