...
import { TestUtils, createTestUtils, formatUSDC, formatDuration } from './testUtils';
...
╚════════════════════════════════════════════════════════════════════════════╝
  ...
  const testUtils = createTestUtils(API_URL);
  const results = {
    totalTests: 0,
    passedTests: 0,
    failedTests: 0,
  ...
  try {
    ...
    const market = await testUtils.createMarket(
    ...
    results.passedTests++;
    console.log(
      `✅ Market created: ${market.id.slice(0, 20)}...`
    );
  ...
  if (testMarketId) {
    try {
      ...
      console.log(`   Total Stake: ${formatUSDC(market.total_stake)}`);
  ...
  if (testMarketId) {
    for (const staker of testStakers) {
      try {
        ...
        const opinion = await testUtils.stakeOpinion(
          testMarketId,
          5_000_000, // $5
          `Bullish on Bitcoin, staker ${staker.slice(0, 10)}`,
          staker
        );
        results.totalTests++;
        results.passedTests++;
        console.log(`✅ Opinion staked by ${staker}`);
        console.log(`   Amount: ${formatUSDC(opinion.amount)}`);
  ...
  for (const staker of testStakers) {
    try {
      ...
      console.log(`   Total Staked: ${formatUSDC(portfolio.total_staked)}`);
      console.log(`   Total Won: ${formatUSDC(portfolio.total_prize_won)}`);
